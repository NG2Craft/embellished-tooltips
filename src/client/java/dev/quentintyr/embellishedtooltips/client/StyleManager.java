...
public class StyleManager {
   ...
   public static Optional<TooltipStyle> getStyleFor(ItemStack stack) {
      ...
      return preset == null ? defaultStyle()
            : Optional.of((new TooltipStyle.Builder()).withPanel((TooltipPanel) preset.getPanel().orElse(DEFAULT_PANEL))
                  .withFrame((TooltipFrame) preset.getFrame().orElse(DEFAULT_FRAME))
                  .withIcon((TooltipIcon) preset.getIcon().orElse(DEFAULT_ICON)).withEffects(preset.getEffects())
                  .build());
   }

   public static Optional<TooltipStyle> defaultStyle() {
      return Optional.of((new TooltipStyle.Builder()).withPanel(DEFAULT_PANEL).withFrame(DEFAULT_FRAME)
