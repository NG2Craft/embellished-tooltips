...
public class EnderParticle extends TooltipParticle {
   ...
   public EnderParticle(int centerColor, int edgeColor, float lifetime, Vec2 center, float range) {
      ...
      this.start = center.m_165910_(new Vec2((float)Math.cos((double)rot) * range, (float)Math.sin((double)rot) * range));
   }
   ...
   public void renderParticle(TooltipContext context, float lifetime) {
      ...
      float scale = (mod < 0.4F ? (float)Math.pow((double)(mod / 0.4F), 3.0D) : (mod < 0.9F ? 1.0F - (float)Math.pow((double)((mod - 0.4F) / 0.5F), 3.0D) : 0.0F)) * 1.2F;
      context.push(() -> {
         context.translate(Mth.m_14179_((float)Math.pow((double)mod, 4.0D), this.start.f_82470_, this.position.f_82470_), Mth.m_14179_((float)Math.pow((double)mod, 4.0D), this.start.f_82471_, this.position.f_82471_), 0.0F);
