...
public class RimLightingEffect implements TooltipEffect {
   ...
   public void render(TooltipContext context, Vec2 pos, Point size) {
      ...
      buffer.m_252986_(matrix4f, start.f_82470_ + width, start.f_82471_ + (float) size.y - width, 0.0F)
      ...
      buffer.m_252986_(matrix4f, start.f_82470_, start.f_82471_, 0.0F).m_193479_(this.START).m_5752_();
      buffer.m_252986_(matrix4f, start.f_82470_ + width, start.f_82471_ + width, 0.0F).m_193479_(this.END).m_5752_();
      buffer.m_252986_(matrix4f, start.f_82470_ + (float) size.x - width, start.f_82471_ + width, 0.0F)
            .m_193479_(this.END).m_5752_();
      buffer.m_252986_(matrix4f, start.f_82470_ + (float) size.x, start.f_82471_, 0.0F).m_193479_(this.START).m_5752_();
      buffer = context.bufferSource().m_6299_(RenderType.m_286086_());
      buffer.m_252986_(matrix4f, start.f_82470_ + (float) size.x - width, start.f_82471_ + width, 0.0F)
            .m_193479_(this.END).m_5752_();
      buffer.m_252986_(matrix4f, start.f_82470_ + (float) size.x - width, start.f_82471_ + (float) size.y - width, 0.0F)
            .m_193479_(this.END).m_5752_();
      buffer.m_252986_(matrix4f, start.f_82470_ + (float) size.x, start.f_82471_ + (float) size.y, 0.0F)
      ...
      if (context.time() - this.lastParticle >= 0.1F) {
         ...
         switch (edge) {
            case 1:
               ...
               break;
            case 2:
               var10000 = new Vec2(start.f_82470_ + (float) size.x, start.f_82471_ + (float) size.y * mod);
               break;
            case 3:
               var10000 = new Vec2(start.f_82470_ + (float) size.y * mod, start.f_82471_);
               break;
            default:
               var10000 = new Vec2(start.f_82470_ + (float) size.y * mod, start.f_82471_ + (float) size.y);
         }

         Vec2 from = var10000;
         this.particles.add(new LineParticle(this.PARTICLE_CENTER, this.PARTICLE_EDGE, 1.0F, from,
