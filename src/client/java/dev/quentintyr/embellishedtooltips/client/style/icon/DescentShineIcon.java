...
public class DescentShineIcon implements TooltipIcon {
   ...
   public void render(TooltipContext context, int x, int y) {
      float scale = (double) context.time() < 0.25D
            ? (1.0F - (float) Math.pow((double) (1.0F - context.time() * 4.0F), 3.0D)) * 1.5F
            : ((double) context.time() < 0.5D
                  ? 1.5F - (1.0F - (float) Math.pow((double) (1.0F - (context.time() - 0.25F) * 4.0F), 3.0D)) * 0.25F
                  : 1.25F);
      float scale2 = (double) (context.time() * 0.5F) < 0.25D
            ? (1.0F - (float) Math.pow((double) (1.0F - context.time() * 0.5F * 4.0F), 3.0D)) * 1.5F
            : ((double) (context.time() * 0.5F) < 0.5D ? 1.5F
                  - (1.0F - (float) Math.pow((double) (1.0F - (context.time() * 0.5F - 0.25F) * 4.0F), 3.0D)) * 0.25F
                  : 1.25F);
      ...
      if (context.time() - this.lastParticle >= 0.1F) {
         ...
         this.particles.add(new SparkleParticle(this.PARTICLE_CENTER_COLOR, this.PARTICLE_EDGE_COLOR, 1.5F,
               new Vec2(0.0F, 0.0F),
               new Vec2((float) Math.cos((double) rotation) * 10.0F, (float) Math.sin((double) rotation) * 10.0F)));
      }

      context.renderParticles(this.particles);
      context.renderItem(new Vector3f(0.0F,
            360.0F * (1.0F - (float) Math.pow((double) (1.0F - Math.min(1.0F, context.time() * 2.0F)), 3.0D)), 0.0F),
            new Vector3f(1.0F));
   }
