...
public class DescentShineIcon implements TooltipIcon {
   ...
   public void render(TooltipContext context, int x, int y) {
      ...
      float scale2 = (double)(context.time() * 0.5F) < 0.25D ? (1.0F - (float)Math.pow((double)(1.0F - context.time() * 0.5F * 4.0F), 3.0D)) * 1.5F : ((double)(context.time() * 0.5F) < 0.5D ? 1.5F - (1.0F - (float)Math.pow((double)(1.0F - (context.time() * 0.5F - 0.25F) * 4.0F), 3.0D)) * 0.25F : 1.25F);
      ...
      for(rotation = 0.0F; rotation < 2.0F; rotation += 0.2F) {
         ...
         first = new Vector2f((float)(Math.cos(d1) * (double)dist2), (float)(Math.sin(d1) * (double)dist2));
         second = new Vector2f((float)(Math.cos(d2) * (double)dist2), (float)(Math.sin(d2) * (double)dist2));
         matrix4f = context.pose().m_85850_().m_252922_();
         vertexconsumer = context.bufferSource().m_6299_(RenderType.m_286086_());
         vertexconsumer.m_252986_(matrix4f, centerX, centerY, 0.0F).m_193479_(this.START_COLOR).m_5752_();
         vertexconsumer.m_252986_(matrix4f, centerX, centerY, 0.0F).m_193479_(this.START_COLOR).m_5752_();
      ...
      for(rotation = 0.0F; rotation < 2.0F; rotation += 0.2F) {
         d1 = (double)context.time() + 3.141592653589793D * (double)(rotation + 0.05F + mod);
         d2 = (double)context.time() + 3.141592653589793D * (double)(rotation - 0.05F - mod);
         first = new Vector2f((float)(Math.cos(d1) * (double)dist1), (float)(Math.sin(d1) * (double)dist1));
         second = new Vector2f((float)(Math.cos(d2) * (double)dist1), (float)(Math.sin(d2) * (double)dist1));
         matrix4f = context.pose().m_85850_().m_252922_();
         vertexconsumer = context.bufferSource().m_6299_(RenderType.m_286086_());
         vertexconsumer.m_252986_(matrix4f, centerX, centerY, 0.0F).m_193479_(this.CENTER_COLOR).m_5752_();
         vertexconsumer.m_252986_(matrix4f, centerX, centerY, 0.0F).m_193479_(this.CENTER_COLOR).m_5752_();
         vertexconsumer.m_252986_(matrix4f, first.x, first.y, 0.0F).m_193479_(this.END_COLOR).m_5752_();
         vertexconsumer.m_252986_(matrix4f, second.x, second.y, 0.0F).m_193479_(this.END_COLOR).m_5752_();
      }
      ...
      if (context.time() - this.lastParticle >= 0.1F) {
         ...
         this.particles.add(new SparkleParticle(this.PARTICLE_CENTER_COLOR, this.PARTICLE_EDGE_COLOR, 1.5F, new Vec2(0.0F, 0.0F), new Vec2((float)Math.cos((double)rotation) * 10.0F, (float)Math.sin((double)rotation) * 10.0F)));
      }

      context.renderParticles(this.particles);
      context.renderItem(new Vector3f(0.0F, 360.0F * (1.0F - (float)Math.pow((double)(1.0F - Math.min(1.0F, context.time() * 2.0F)), 3.0D)), 0.0F), new Vector3f(1.0F));
   }
